...
export const SeamSimulator = ({ showHeader = false }) => {
  ...
  return (
    <div className="space-y-4 max-w-7xl mx-auto text-foreground">
      ...
      {/* Guide Note */}
      <div className="p-4 rounded-xl bg-card border border-border shadow-sm text-xs space-y-2 text-muted-foreground">
        <h3 className="font-bold text-foreground flex items-center gap-1.5 text-sm">
          <HelpCircle className="w-4 h-4 text-blue-400" />
          ラプソード 3D シミュレータの使い方 & 物理仕様
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-1">
          <div className="p-3 rounded-xl bg-muted/30 border border-border/60 space-y-1">
            <h4 className="font-bold text-foreground">1. 時計盤チルト (Spin Axis)</h4>
            <p>
              投手視点（ラプソード標準）で、12:00（0°）が真上へのバックスピン（ストレート）。1:15〜1:30が右投手の典型的なシュート成分、6:00が真下へのトップスピン（ドロップカーブ）です。
            </p>
          </div>
          <div className="p-3 rounded-xl bg-muted/30 border border-border/60 space-y-1">
          ...
          <div className="p-3 rounded-xl bg-muted/30 border border-border/60 space-y-1">
            <h4 className="font-bold text-foreground">3. 3Dシーム (4シーム/2シーム/1シーム)</h4>
            <p>
              縫い目（シーム）の初期姿勢を3種類から選択可能。回転中の縫い目の見え方（Seam Shifted Wake効果や残像パターン）を極スローでじっくり観察・比較できます。
            </p>
          </div>
        </div>
      </div>
